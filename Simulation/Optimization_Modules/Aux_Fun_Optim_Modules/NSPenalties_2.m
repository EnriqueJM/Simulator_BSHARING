function [E_NSP_i,exEmpty,exFull] = NSPenalties_2(m_i, req_h, ret_h, Cap_i, beta)
% NSPENALTIES - ver. 1.1
...
exFull = 0;
    ...
    if req_h ~= 0 || ret_h ~= 0
        exEmpty = expectEmpty(m_i, req_h, ret_h);
        exFull = expectFull(m_i, req_h, ret_h, Cap_i);

%         E_NSP_i = beta(1) * req_h * probEmpty(m_i, req_h, ret_h) ...
%             + beta(2) * ret_h * probFull(m_i, req_h, ret_h, Cap_i);
        E_NSP_i = beta(1) * exEmpty ...
            + beta(2) * exFull;
    end
