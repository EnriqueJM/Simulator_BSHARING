function [PwTeams, T_cost, y_end, UT] = pwiseAssign_bikSB_twdow(t, vRepoPool, vStations, vBikes, param)
...
t_wndw = 5*60; %% Time window to evaluate the demand change [in min]
...
%%% STEP 2.1: CALCULATE UTILITY MATRIX
    ...
    for istat=1:numel(vStations)
        ...
        %%% VERY IMPORTANT
        if (t+t_wndw)<=numel(vStations{istat}.accRequests)
            req_T = vStations{istat}.accRequests(t+t_wndw)...
                - vStations{istat}.accRequests(t);
            ret_T = vStations{istat}.accReturns(t+t_wndw)...
                - vStations{istat}.accReturns(t);
        else
            t_add = t+t_wndw-numel(vStations{istat}.accRequests);
            
            req_T = vStations{istat}.accRequests(end)...
                + vStations{istat}.accRequests(t_add)...
                - vStations{istat}.accRequests(t);
            ret_T = vStations{istat}.accReturns(end)...
                + vStations{istat}.accReturns(t_add)...
                - vStations{istat}.accReturns(t);
        end
        %        
