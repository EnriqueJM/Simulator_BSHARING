function [vRepo_Out, vStat_Out] = ...
                    optimRepoRouteDual_bikSB_Tfix(vRepoTeams, vStations, vBikes,...
                    param)
...
%   - vRepo_Out -> {Array Vx1} Array of repo teams class objects.
    ...
    t_step = 24; % Fixed time step [in min]
    ...
    while Timer<timeCycle
        ...
        UT = zeros(numel(vRepo_Out), numel(vStat_Out), param.TotalTime);
...
%                     y_end(iteam, istat) = min([-y_i(istat), C_v-y_v]);
                end
                ...
                if y_end(iteam, istat)~=0
                    % Considers only activities fitting into the time step.
                    if T_cost(iteam, istat)<=t_step
                        UT(iteam, istat, Timer) = E_NSP_ini - E_NSP_end;
                    else
                        UT(iteam, istat, Timer) = -99999;
                    end
                else
                    UT(iteam, istat, Timer) = -99999; % Inf or NaN not possible.
                end
            end
        end
        ...
        PwTeams = matchpairs(UT(:,:,Timer),-99999,'max');
        ...
        for i=1:size(PwTeams,1)
            ...
            vRepo_Out{iteam}.taskUtility(end+1) = UT(iteam,istat,Timer);
        ...
        %% UPDATE TIMER   
        %
        % Update timer adding one time step more.
        Timer = Timer + t_step;
